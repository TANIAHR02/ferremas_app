...
for /f "tokens=*" %%i in ('curl -s -X POST http://localhost:8082/api/webpay-simulation/simulate/init -H "Content-Type: application/json" -d "{\"pedidoId\": 99999, \"monto\": 15000.0, \"metodoPago\": \"WEBPAY\"}"') do set RESPONSE=%%i
...
curl -X GET http://localhost:8082/api/webpay-simulation/simulate/status/%TOKEN%
echo.
echo.
...
echo http://localhost:8082/api/webpay-simulation/simulate/%TOKEN%
echo.
